...
router.get('/issue', function(req, res){
  ...
  if(_coordinates === ''){
	  if( _issue === '')
	  {
		  console.log('_coordinates null 1 '+dateFormat(_startdate, "isoDateTime")+' - '+dateFormat(_enddate, "isoDateTime")+' - '+_sort+' - '+_limit);
		  ...
		  Issue.find({"create_at":{$gte:dateFormat(_startdate, "isoDateTime");_startdate, $lt:dateFormat(_enddate, "isoDateTime")}
							},function(err, issue){
			res.send(err);
