...
router.get('/issue', function(req, res){
  ...
  if(_coordinates === ''){
	  if( _issue === '')
	  {
		  console.log('_coordinates null 1 '+_startdate.toString()+' - '+_enddate.toString()+' - '+_sort+' - '+_limit);
		  ...
		  Issue.find({"create_at":{$gte:new Date(_startdate.toString()), $lt:new Date(_enddate.toString())}
							},function(err, issue){
